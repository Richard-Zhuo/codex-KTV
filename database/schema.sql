...
CREATE TABLE room_issue_change_requests (
  id bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
  room_id bigint NOT NULL REFERENCES rooms(id),
  room_issue_id bigint NOT NULL REFERENCES room_issues(id) ON DELETE RESTRICT,
  change_type varchar(20) NOT NULL CHECK (change_type = '恢复空房'),
  from_status varchar(30) NOT NULL,
  requested_status varchar(30) NOT NULL CHECK (requested_status = '空闲'),
  ...
  CHECK (
    ...
    OR (status = '已失效' AND decided_at IS NOT NULL)
  )
);
