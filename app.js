...
function migrateDemoState(next) {
  ...
  next.capabilitySchemaVersion = 2;
...
function repaymentReviewMarkup(o) {
  ...
  return rows.map(request=>{const canReview=allowedPermission('credit.repay.approve')&&request.submittedById!==state.user;return `<div class="notice"><div class="split"><b>回款 ${money(request.amount)} · ${esc(request.method)}</b><span class="badge">待审核</span></div><p>登记人 ${esc(request.submittedBy)} · ${date(request.submittedAt)}</p>${canReview?btn('审核回款','reviewRepayment',`data-id="${o.id}" data-request="${request.id}"`,'primary full'):request.submittedById===state.user?'<span class="badge">不能审核本人申请</span>':'<span class="badge">需要回款审核权限</span>'}</div>`;}).join('');
}
...
function giftRequestCards() {
  ...
  return rows.map(({o,request})=>`<article class="panel"><div class="split"><h3>${o.room} · ${product(request.product).name}</h3><span class="badge">待确认</span></div><p>${request.halves} 个半打，共 ${request.bottles} 支 · 申请人 ${esc(request.requestedBy)}</p>${request.requestedById===state.user?'<span class="badge">不能审核本人申请</span>':allowedPermission('gift.approve')?btn('进入账单处理','order',`data-id="${o.id}"`):'<span class="badge">需要赠酒水审核权限</span>'}</article>`).join('') || '<p class="muted">暂无超额赠酒水申请。</p>';
}
function roundingReviewCards() {
  const rows=state.orders.filter(order=>order.roundingReview?.status==='待审核');
  return rows.map(order=>{const canReview=allowedPermission('rounding.approve')&&order.roundingReview.submittedById!==state.user;return `<article class="panel"><div class="split"><h3>${esc(order.room)} · 特殊情况 ${money(order.roundingReview.amount)}</h3><span class="badge">待审核</span></div><p>${esc(order.roundingReview.note)} · 提交人 ${esc(order.roundingReview.submittedBy)}</p><p class="muted">${date(order.roundingReview.submittedAt)}</p>${canReview?btn('查看并审核','reviewRounding',`data-id="${order.id}"`):order.roundingReview.submittedById===state.user?'<span class="badge">不能审核本人申请</span>':'<span class="badge">需要特殊差额审核权限</span>'}</article>`;}).join('') || '<p class="muted">暂无特殊差额待审核。</p>';
}
function inventoryReviewCards() {
  const rows=(state.inventoryReviews||[]).filter(request=>request.status==='待审核');
  return rows.map(request=>{const label=request.kind==='consumable'?(CONSUMABLES.find(item=>item.id===request.product)?.name||request.product):product(request.product).name;const unit=request.kind==='consumable'?(state.consumables?.[request.product]?.unit||'份'):'支';const opened=request.kind==='consumable'?` · 已开封 ${request.openedBefore||0} → ${request.openedAfter||0}`:'';const canReview=allowedPermission('inventory.approve')&&request.submittedById!==state.user;return `<article class="panel"><div class="split"><h3>${esc(label)} · ${esc(request.source)}</h3><span class="badge">待审核</span></div><p>${request.before??'未建账'} → ${request.after} ${esc(unit)}${opened}</p><p>${esc(request.reason)} · 提交人 ${esc(request.submittedBy)}</p>${canReview?btn('审核库存盘点','reviewInventory',`data-id="${request.id}"`,'primary full'):request.submittedById===state.user?'<span class="badge">不能审核本人申请</span>':'<span class="badge">需要库存审核权限</span>'}</article>`;}).join('')||'<p class="muted">暂无库存盘点待审核。</p>';
}
function incidentResolutionReviewCards() {
  ...
  return rows.map(({incident,request})=>{const canReview=allowedPermission('incident.resolve.approve')&&request.submittedById!==state.user;return `<article class="panel"><div class="split"><h3>${esc(incident.room)} · ${esc(incident.type)}</h3><span class="badge">恢复待审核</span></div><p>${esc(request.result)}</p><p class="muted">提交 ${esc(request.submittedBy)} · ${date(request.submittedAt)}</p>${canReview?btn('审核处理结果','reviewIncidentResolution',`data-id="${incident.id}" data-request="${request.id}"`,'primary full'):request.submittedById===state.user?'<span class="badge">不能审核本人申请</span>':'<span class="badge">需要客诉／异常恢复审核权限</span>'}</article>`;}).join('')||'<p class="muted">暂无客诉／异常恢复待审核。</p>';
}
...
function incidentPage() {
  ...
  const body=rows.map(row=>{
    ...
    const canReview=pending&&allowedPermission('incident.resolve.approve')&&pending.submittedById!==state.user;
    ...
    const review=pending?`<div class="notice"><b>处理结果待另一名员工审核</b><p>${esc(pending.result)}</p><p>${esc(pending.note)}</p><p class="muted">提交 ${esc(pending.submittedBy)} · ${date(pending.submittedAt)}</p>${canReview?btn('审核处理结果','reviewIncidentResolution',`data-id="${row.id}" data-request="${pending.id}"`,'primary full'):pending.submittedById===state.user?'<span class="badge">不能审核本人申请</span>':'<span class="badge">需要客诉／异常恢复审核权限</span>'}</div>`:'';
...
function showOrder(id) {
  ...
  const requests=(o.giftRequests||[]).filter(request=>request.status==='待确认').map(request=>{const canReview=allowedPermission('gift.approve')&&request.requestedById!==state.user;return `<div class="notice"><b>超额赠酒水待审核</b><p>${product(request.product).name} ${request.halves}个半打，共${request.bottles}支 · ${esc(request.requestedBy)}申请</p>${canReview?`<div class="inline-actions">${btn('批准赠送','approveGift',`data-id="${id}" data-request="${request.id}"`,'primary')}${btn('驳回','rejectGift',`data-id="${id}" data-request="${request.id}"`,'danger')}</div>`:request.requestedById===state.user?'<small>提交人不能审核本人申请，请切换其他有权限的员工。</small>':'<small>请另一名有赠酒水审核权限的员工处理后再结账。</small>'}</div>`;}).join('');
...
function inventoryDialog() {
  ...
  openDialog('库存 · 酒水与消耗品',`<p class="notice">酒水按支数统计；消耗品按包数／份数统计，并记录已开封数量。每次建账或盘点需另一名有库存审核权限的员工批准，审核前不改变账面库存。</p><h3>待审核盘点</h3>${inventoryReviewCards()}<h3>酒水库存</h3>${drinkRows}<h3>消耗品库存</h3>${consumableRows}<h3>最近库存流水</h3>${logs||'<p class="muted">暂无流水</p>'}`);
}
...
document.addEventListener('click',e=>{
  ...
  try {
    ...
    else if(a==='approveGift'||a==='rejectGift'){const request=Number(target.dataset.request), gift=(state.orders.find(o=>o.id===id)?.giftRequests||[]).find(item=>item.id===request);if(!gift||gift.status!=='待确认')throw Error('赠酒水申请已处理');if(gift.requestedById===state.user)throw Error('提交人不能审核本人申请，请切换其他有审核权限的身份');openDialog(a==='approveGift'?'批准超额赠酒水':'驳回超额赠酒水',`<p>${product(gift.product).name} ${gift.halves}个半打，共${gift.bottles}支。</p><p>申请人：${esc(gift.requestedBy)}</p>${a==='rejectGift'?'<label>驳回原因<textarea name="decisionNote" maxlength="300" rows="3" required></textarea></label>':'<label>审核备注（选填）<textarea name="decisionNote" maxlength="300" rows="2"></textarea></label>'}`,a==='approveGift'?'确认批准':'确认驳回',a,{order:id,request});}
    else if(a==='reviewRounding'){const o=state.orders.find(o=>o.id===id), review=o?.roundingReview;if(!review||review.status!=='待审核')throw Error('这笔特殊差额已经处理');if(review.submittedById===state.user)throw Error('提交人不能审核本人申请，请切换其他有审核权限的身份');openDialog('审核特殊差额',`<div class="quote"><span>${esc(o.room)} · 结账差额</span><strong>${money(review.amount)}</strong></div><p>提交人：${esc(review.submittedBy)} · ${date(review.submittedAt)}</p><p class="notice">特殊情况说明：${esc(review.note)}</p><p class="muted">本次审核只确认差额原因已核实，不会追加扣款或改变已完成的结账。</p><label>审核备注（选填）<textarea name="decisionNote" maxlength="300" rows="2"></textarea></label>${btn('驳回差额说明','rejectRoundingDialog',`data-id="${o.id}"`,'danger full')}`,'批准特殊差额','approveRounding',{order:id});}
